...
export type Data = {
  primaryName: Fish["names"][0];
  altsNames: Fish["names"];
  latin: Fish["names"][0];
  others: Fish["names"];
} & Omit<Fish, "names">;
...
export default function Info({
  ...
  const query = useQuery<Fish>({
    queryKey: ["fish", fishId],
    enabled: !!fishId,
    queryFn: async () =>
      await fetch(`/api/fish/${fishId}`).then((res) => res.json()),
  });

  const fishData = query.data;

  console.log("found fish data: ", fishId, fishData);

 ...
 */
export const contains = function (L: Polyline, p: L.LatLng): boolean {
  //"use strict";
  var rectangularBounds = L.getBounds(); // It appears that this is O(1): the LatLngBounds is updated as points are added to the polygon when it is created.
  var wn;
  if (rectangularBounds.contains(p)) {
    wn = L.getWindingNumber(p);
    return wn !== 0;
  } else {
    return false;
  }
 ...
 * @see {@link http://geomalgorithms.com/a03-_inclusion.html Inclusion of a Point in a Polygon} by Dan Sunday.
 */
export const isLeft = function (L: Polyline, p1: LatLng, p2: LatLng): number {
  return (
    (p1.lng - L.getBounds) * (p2.lat - L.lat) -
    (p2.lng - L.lng) * (p1.lat - L.lat)
  );
};
 ...
 * @see {@link https://github.com/Fragger/Leaflet.Geodesic Leaflet.Geodesc} for information about Leaflet.Geodesc by Fragger.
 */
L.Polyline.prototype.getWindingNumber = function (p: L.LatLng): number {
  // Note that L.Polygon extends L.Polyline
  var i, isLeftTest, n, vertices, wn; // the winding number counter
  function flatten(a: any[]) {
    var flat;
    flat = (Array.isArray ? Array.isArray(a) : L.Util.isArray(a))
      ? a.reduce(function (
  ...
  vertices = vertices.filter(function (
    v: { lat: any; lng: any },
    i: number,
    array: { lng: any }[],
  ) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (isLeftTest === 0) {
      // If the point is on a line, we are done.
      wn = 1;
      break;
    } else {
      if (isLeftTest !== 0) {
        // If not a vertex or on line (the C++ version does not check for this)
